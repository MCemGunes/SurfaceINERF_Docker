...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsNaiveHW(
    const torch::Tensor &points,
    const torch::Tensor &ellipse_params,
    const torch::Tensor &cutoff_thres,
    const torch::Tensor &radii,
    ...
    const int points_per_pixel)
{
  if (points.is_cuda())
  {
    ...
    CHECK_CUDA(cloud_to_packed_first_idx);
    CHECK_CUDA(num_points_per_cloud);
    return RasterizePointsNaiveHWCuda(
        points,
        ellipse_params,
        cutoff_thres,
        radii,
...
//          compute the composition weight exp(-Qvalue)
//  occupancy: float32  (N, S, S) giving the occupancy 0/1 at the pixel
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsNaiveNew(
    const torch::Tensor &points,
    const torch::Tensor &ellipse_params,
    const torch::Tensor &cutoff_thres,
    const torch::Tensor &radii,
    ...
    const int num_shaping_points)
{
  if (points.is_cuda())
  {
    ...
    CHECK_CUDA(cloud_to_packed_first_idx);
    CHECK_CUDA(num_points_per_cloud);
    return RasterizePointsNaiveNewCuda(
        points,
        ellipse_params,
        cutoff_thres,
        radii,
...
// ****************************************************************************
// *                          NAIVE RASTERIZATION New  HW version             *
// ****************************************************************************
#ifdef WITH_CUDA
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor>
...
//          compute the composition weight exp(-Qvalue)
//  occupancy: float32  (N, S, S) giving the occupancy 0/1 at the pixel
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsNaiveNewHW(
    const torch::Tensor &points,
    const torch::Tensor &ellipse_params,
    const torch::Tensor &cutoff_thres,
    const torch::Tensor &radii,
    ...
    const int num_shaping_points)
{
  if (points.is_cuda())
  {
    ...
    CHECK_CUDA(cloud_to_packed_first_idx);
    CHECK_CUDA(num_points_per_cloud);
    return RasterizePointsNaiveNewHWCuda(
        points,
        ellipse_params,
        cutoff_thres,
        radii,
...
// ****************************************************************************
// *                          NAIVE RASTERIZATION New  HW version V2          *
// ****************************************************************************
#ifdef WITH_CUDA
...
//          compute the composition weight exp(-Qvalue)
//  occupancy: float32  (N, S, S) giving the occupancy 0/1 at the pixel
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsNaiveNewHWD(
    const torch::Tensor &points,
    const torch::Tensor &ellipse_params,
    const torch::Tensor &cutoff_thres,
    const torch::Tensor &radii,
    ...
    const int num_shaping_points)
{
  if (points.is_cuda())
  {
    ...
    CHECK_CUDA(cloud_to_packed_first_idx);
    CHECK_CUDA(num_points_per_cloud);
    return RasterizePointsNaiveNewHWDCuda(
        points,
        ellipse_params,
        cutoff_thres,
        radii,
    ...
    const int num_shaping_points)
{
  ...
  else if (bin_size == 8888)
  {
    // Use the new per-pixel implementation
    return RasterizePointsNaiveNew(
        points,
        ellipse_params,
        cutoff_thres,
        radii,
...
// ****************************************************************************
// *                         MAIN ENTRY POINT For H*W version                 *
// ****************************************************************************
...
//              corresponding to idx map. EWA ~ exp(-0.5Q)
//  occupance_map: a map indicating whether this pixel is occupied or not
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsHW(
    const torch::Tensor &points,
    const torch::Tensor &ellipse_params,
    const torch::Tensor &cutoff_thres,
    const torch::Tensor &radii,
    ...
    const int num_shaping_points)
{
  ...
  at::TensorArg points_t{points, "points", 1},
      ...
      cloud_to_packed_first_idx_t{
          cloud_to_packed_first_idx, "cloud_to_packed_first_idx", 5},
      num_points_per_cloud_t{num_points_per_cloud, "num_points_per_cloud", 6};
  at::CheckedFrom c = "RasterizePointsHW";
  torch::checkDim(c, points_t, 2);
  torch::checkSize(c, points_t, 1, 3);
  torch::checkSize(c, radii_t, {points_t->size(0), 2});
  torch::checkSize(c, ellipse_params_t, {points_t->size(0), 3});
  ...
  {
    // Use the new per-pixel implementation
    return RasterizePointsNaiveNewHW(
        points,
        ellipse_params,
        cutoff_thres,
        radii,
...
// ****************************************************************************
// *            MAIN ENTRY POINT For H*W version V2                           *
// ****************************************************************************
...
//              corresponding to idx map. EWA ~ exp(-0.5Q)
//  occupance_map: a map indicating whether this pixel is occupied or not
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> RasterizePointsHWD(
    const torch::Tensor &points,
    const torch::Tensor &ellipse_params,
    const torch::Tensor &cutoff_thres,
    const torch::Tensor &radii,
    ...
    const int num_shaping_points)
{
  ...
  if (bin_size == 0)
  {
    throw std::invalid_argument( "received wrong bin_size value." );
    // Use the naive per-pixel implementation
//    return RasterizePointsNaiveHW(
//        points,
//        ellipse_params,
...
//        points_per_pixel);
  ...
  {
    // Use the new per-pixel implementation
    return RasterizePointsNaiveNewHWD(
        points,
        ellipse_params,
        cutoff_thres,
        radii,
