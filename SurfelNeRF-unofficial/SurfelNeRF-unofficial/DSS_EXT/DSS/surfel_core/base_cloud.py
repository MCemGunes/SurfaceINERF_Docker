...
from .base_types import Device, make_device
from . import base_utils as struct_utils

"""
We rewrite the Pointclouds to support:
1) It can modify the points dir and points color using the same structure as normals of features!
"""
...
class Pointclouds:
    ...
    def __init__(self, points, normals=None, features=None, colors=None, dirs=None, weights=None) -> None:
        """
        Args:
            points:
                Can be either
            ...
            colors:
                Same as features
            dirs:
                points direction! Same as features
        ...
        self._C_weights = None  # number of channels in the weights
        ...
        self._colors_packed = None  # (sum(P_n), 3)
        self._dirs_packed = None  # (sum(P_n), 3)
        self._weights_packed = None  # (sum(P_n), C_{weight})
        ...
        self._colors_padded = None  # (N, max(P_n), 3)
        self._dirs_padded = None  # (N, max(P_n), 3)
        self._weights_padded = None  # (N, max(P_n), C_{weight})
        ...
        # parse colors
        colors_parsed = self._parse_auxiliary_input(colors)
        self._colors_list, self._colors_padded, colors_C = colors_parsed
        if colors_C is not None and colors_C != 3:
            raise ValueError("Colors are expected to be 3-dimensional")

        # parse dirs  # dirs indicates the direction of points from the camera position, which may be helpful for novel view synthesis
        dirs_parsed = self._parse_auxiliary_input(dirs)
        self._dirs_list, self._dirs_padded, dirs_C = dirs_parsed
        if dirs_C is not None and dirs_C != 3:
            raise ValueError("Point dirs are expected to be 3-dimensional")

        # parse weights
        weights_parsed = self._parse_auxiliary_input(weights)
        self._weights_list, self._weights_padded, weights_C = weights_parsed
        if weights_C is not None:
            self._C_weights = weights_C
    ...
    def __getitem__(self, index) -> "Pointclouds":
        ...
        """
        normals, features, colors, dirs, weights = None, None, None, None, None
        normals_list = self.normals_list()
        features_list = self.features_list()
        colors_list = self.colors_list()
        dirs_list = self.dirs_list()
        ...
        if isinstance(index, int):
            ...
            if colors_list is not None:
                colors = [colors_list[i] for i in index]
            if dirs_list is not None:
                dirs = [dirs_list[i] for i in index]
            if weights_list is not None:
                weights = [weights_list[i] for i in index]
        ...
        return self.__class__(points=points, normals=normals, features=features,
                              colors=colors, dirs=dirs, weights=weights)
    ...
    def weights_list(self) -> Optional[List[torch.Tensor]]:
        """
        Get the list representation of the weights,
        or None if there are no weights.

        Returns:
            list of tensors of weights of shape (P_n, C).
        """
        if self._weights_list is None:
            if self._weights_padded is None:
                # No weights provided so return None
                return None
            self._weights_list = struct_utils.padded_to_list(
                self._weights_padded, self.num_points_per_cloud().tolist()
            )
        return self._weights_list

    def colors_list(self) -> Optional[List[torch.Tensor]]:
        """
        Get the list representation of the colors,
        or None if there are no colors.

        Returns:
            list of tensors of colors of shape (P_n, 3).
        """
        if self._colors_list is None:
            if self._colors_padded is None:
                # No colors provided so return None
                return None
            self._colors_list = struct_utils.padded_to_list(
                self._colors_padded, self.num_points_per_cloud().tolist()
            )
        return self._colors_list

    def dirs_list(self) -> Optional[List[torch.Tensor]]:
        """
        Get the list representation of the pcd dirs,
        or None if there are no pcd dirs.

        Returns:
            list of tensors of colors of shape (P_n, 3).
        """
        if self._dirs_list is None:
            if self._dirs_padded is None:
                # No pcd dirs provided so return None
                return None
            self._dirs_list = struct_utils.padded_to_list(
                self._dirs_padded, self.num_points_per_cloud().tolist()
            )
    ...
    def weights_packed(self) -> Optional[torch.Tensor]:
        """
        Get the packed representation of the weights.

        Returns:
            tensor of weights of shape (sum(P_n), C),
            or None if there are no weights
        """
        self._compute_packed()
        return self._weights_packed

    def colors_packed(self) -> Optional[torch.Tensor]:
        """
        Get the packed representation of the colors.

        Returns:
            tensor of colors of shape (sum(P_n), 3),
            or None if there are no colors
        """
        self._compute_packed()
        return self._colors_packed

    def dirs_packed(self) -> Optional[torch.Tensor]:
        """
        Get the packed representation of the pcd dirs.

        Returns:
            tensor of dirs of shape (sum(P_n), 3),
            or None if there are no colors
        """
        self._compute_packed()
        return self._dirs_packed
    ...
    def colors_padded(self) -> Optional[torch.Tensor]:
        """
        Get the padded representation of the colors,
        or None if there are no colors.

        Returns:
            tensor of colors of shape (N, max(P_n), 3).
        """
        self._compute_padded()
        return self._colors_padded

    def dirs_padded(self) -> Optional[torch.Tensor]:
        """
        Get the padded representation of the pcd dirs,
        or None if there are no pcd dirs.

        Returns:
            tensor of dirs of shape (N, max(P_n), 3).
        """
        self._compute_padded()
        return self._dirs_padded

    def weights_padded(self) -> Optional[torch.Tensor]:
        """
        Get the padded representation of the weights,
        or None if there are no weights.

        Returns:
            tensor of weights of shape (N, max(P_n), 3).
        """
        self._compute_padded()
    ...
    def _compute_padded(self, refresh: bool = False):
        ...
        self._normals_padded, self._features_padded, self._colors_padded, self._dirs_padded, self._weights_padded = None, None, None, None, None
        if self.isempty():
            ...
            colors_list = self.colors_list()
            if colors_list is not None:
                self._colors_padded = struct_utils.list_to_padded(
                    colors_list,
                    (self._P, 3),
                    pad_value=0.0,
                    equisized=self.equisized,
            ...
            dirs_list = self.dirs_list()
            if dirs_list is not None:
                self._dirs_padded = struct_utils.list_to_padded(
                    dirs_list,
                    (self._P, 3),
                    pad_value=0.0,
                    equisized=self.equisized,
            ...
            weights_list = self.weights_list()
            if weights_list is not None:
                self._weights_padded = struct_utils.list_to_padded(
                    weights_list,
                    (self._P, self._C_weights),
                    pad_value=0.0,
                    equisized=self.equisized,
                )
    ...
    def _compute_packed(self, refresh: bool = False):
        ...
        if self.isempty():
            ...
            self._colors_packed = None
            self._dirs_packed = None
        ...
        self._normals_packed, self._features_packed, self._colors_packed, self._dirs_packed, self._weights_packed = None, None, None, None, None
        if normals_list is not None:
        ...
        if colors_list is not None:
            colors_list_to_packed = struct_utils.list_to_packed(colors_list)
            self._colors_packed = colors_list_to_packed[0]

        if dirs_list is not None:
            dirs_list_to_packed = struct_utils.list_to_packed(dirs_list)
            self._dirs_packed = dirs_list_to_packed[0]

        if weights_list is not None:
            weights_list_to_packed = struct_utils.list_to_packed(weights_list)
    ...
    def clone(self):
        ...
        new_points, new_normals, new_features, new_colors, new_dirs, new_weights = None, None, None, None, None, None
        if self._points_list is not None:
            ...
            if colors_list is not None:
                new_colors = [f.clone() for f in colors_list]
            if dirs_list is not None:
                new_dirs = [f.clone() for f in dirs_list]
            if weights_list is not None:
                new_weights = [f.clone() for f in weights_list]
            ...
            colors_padded = self.colors_padded()
            dirs_padded = self.dirs_padded()
            weights_padded = self.weights_padded()
            ...
            if colors_padded is not None:
                new_colors = self.colors_padded().clone()
            if dirs_padded is not None:
                new_dirs = self.dirs_padded().clone()
            if weights_padded is not None:
                new_weights = self.weights_padded().clone()
        other = self.__class__(
            points=new_points, normals=new_normals, features=new_features, colors=new_colors, dirs=new_dirs,
            weights=new_weights
        )
    ...
    def detach(self):
        ...
        if self._points_list is not None:
            ...
            if colors_list is not None:
                new_colors = [f.detach() for f in colors_list]
            if dirs_list is not None:
                new_dirs = [f.detach() for f in dirs_list]
            if weights_list is not None:
                new_weights = [f.detach() for f in weights_list]
        elif self._points_padded is not None:
            ...
            if colors_padded is not None:
                new_colors = self.colors_padded().detach()
            if dirs_padded is not None:
                new_dirs = self.dirs_padded().detach()
            if weights_padded is not None:
                new_weights = self.weights_padded().detach()
        other = self.__class__(
            points=new_points, normals=new_normals, features=new_features, colors=new_colors, dirs=new_dirs,
            weights=new_weights
        )
    ...
    def to(self, device: Device, copy: bool = False):
        ...
        if other._N > 0:
            ...
            if other._colors_list is not None:
                other._colors_list = [f.to(device_) for f in other.colors_list()]
            if other._dirs_list is not None:
                other._dirs_list = [f.to(device_) for f in other.dirs_list()]
            if other._weights_list is not None:
                other._weights_list = [f.to(device_) for f in other.weights_list()]
        for k in self._INTERNAL_TENSORS:
            v = getattr(self, k)
            if torch.is_tensor(v):
    ...
    def get_cloud(self, index: int):
        ...
        if weights_list is not None:
            weights = weights_list[index]
        return points, normals, features, colors, dirs, weights
    ...
    def subsample(self, max_points: Union[int, Sequence[int]]) -> "Pointclouds":
        ...
        dirs_list = []
        weights_list = []
        for max_, n_points, points, features, normals, colors, dirs, weights in zip_longest(
            map(int, max_points),
        ...
        return Pointclouds(
            ...
            colors=self.colors_list() and colors_list,
            dirs=self.dirs_list() and dirs_list,
            weights=self.weights_list() and weights_list,
        )
    ...
    def extend(self, N: int):
        ...
        new_points_list, new_normals_list, new_features_list, new_colors_list, new_dirs_list, new_weights_list = [], None, None, None, None, None
        for points in self.points_list():
        ...
        if colors_list is not None:
            ...
            for colors in colors_list:
                new_colors_list.extend(colors.clone() for _ in range(N))
        dirs_list = self.dirs_list()
        if dirs_list is not None:
            new_dirs_list = []
            for dirs in dirs_list:
                new_dirs_list.extend(dirs.clone() for _ in range(N))
        weights_list = self.weights_list()
        if weights_list is not None:
            new_weights_list = []
            for weights in weights_list:
                new_weights_list.extend(weights.clone() for _ in range(N))
        return self.__class__(
            points=new_points_list, normals=new_normals_list, features=new_features_list, colors=new_colors_list,
            dirs=new_dirs_list, weights=new_weights_list
        )

    def update_padded(
        self, new_points_padded, new_normals_padded=None, new_features_padded=None,
        new_colors_padded=None, new_dirs_padded=None, new_weights_padded=None
    ):
        """
        ...
        Args:
            ...
            new_colors_padded: (optional) FloatTensor of shape (N, P, 3)
            new_dirs_padded: (optional) FloatTensor of shape (N, P, 3)
        ...
        if new_colors_padded is not None:
            check_shapes(new_colors_padded, [self._N, self._P, 3])
        if new_dirs_padded is not None:
            check_shapes(new_dirs_padded, [self._N, self._P, 3])
        if new_weights_padded is not None:
            check_shapes(new_weights_padded, [self._N, self._P, self._C_weights])

        new = self.__class__(
            points=new_points_padded,
            normals=new_normals_padded,
            features=new_features_padded,
            colors=new_colors_padded,
            dirs=new_dirs_padded,
            weights=new_weights_padded
        )
        ...
        # copy colors
        if new_colors_padded is None:
            # If no colors are provided, keep old ones (shallow copy)
            new._colors_list = self._colors_list
            new._colors_padded = self._colors_padded
            new._colors_packed = self._colors_packed

        # copy dirs
        if new_dirs_padded is None:
            # If no dirs are provided, keep old ones (shallow copy)
            new._dirs_list = self._dirs_list
            new._dirs_padded = self._dirs_padded
            new._dirs_packed = self._dirs_packed

        # copy weights
        if new_weights_padded is None:
            # If no weights are provided, keep old ones (shallow copy)
            new._weights_list = self._weights_list
            new._weights_padded = self._weights_padded
            new._weights_packed = self._weights_packed
        ...
        # update normals and features, colors, dirs and weights if provided
        if new_normals_padded is not None:
        ...
        if new_colors_padded is not None:
            new._colors_padded = new_colors_padded
            new._colors_list = None
            new._colors_packed = None
        if new_dirs_padded is not None:
            new._dirs_padded = new_dirs_padded
            new._dirs_list = None
            new._dirs_packed = None
        if new_weights_padded is not None:
            new._weights_padded = new_weights_padded
            new._weights_list = None
            new._weights_packed = None
        return new
...
def join_pointclouds_as_batch(pointclouds: Sequence[Pointclouds]):
    ...
    kwargs = {}
    for field in ("points", "normals", "features", "colors", "dirs", "weights"):
