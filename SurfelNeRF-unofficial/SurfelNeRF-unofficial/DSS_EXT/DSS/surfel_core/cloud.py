...
from .base_cloud import Pointclouds as PytorchPointClouds
...
class PointClouds3D(PytorchPointClouds):
    ...
    def __init__(self, points, normals=None, features=None, colors=None, dirs=None, weights=None,
        ...
        """
        super().__init__(points, normals=normals, features=features, colors=colors, dirs=dirs, weights=weights)
        ...
        if to_axis_aligned:
            ...
            rotated_points_packed = world_to_obj_rotate_trans.transform_points(self.points_packed())
            if rotated_points_packed.size(0) == 3 and len(rotated_points_packed.size()) == 3:
                rotated_points_packed = rotated_points_packed[0]
            self.update_points_(rotated_points_packed)
            normals_packed = self.normals_packed()
            if normals_packed is not None:
                rotated_normals_packed = world_to_obj_rotate_trans.transform_normals(normals_packed)
                if rotated_normals_packed.size(0) == 3 and len(rotated_normals_packed.size()) == 3:
                    rotated_normals_packed = rotated_normals_packed[0]
                self.update_normals_(rotated_normals_packed)
    ...
    def subsample_randomly(self, ratio):
        ...
        for b, pts in enumerate(points_list):
            ...
            if colors_list is not None:
                colors_list[b] = colors_list[b][idx]
            if dirs_list is not None:
                dirs_list[b] = dirs_list[b][idx]
            if weights_list is not None:
                weights_list[b] = weights_list[b][idx]

        other = self.__class__(
            points=points_list, normals=normals_list, features=features_list, colors=colors_list, dirs=dirs_list,
...
class PointCloudsFilters(TensorProperties):
    ...
    def filter_with(self, point_clouds: PointClouds3D, filter_names: Tuple[str]):
        ...
        colors = point_clouds.colors_padded()
        if colors is not None:
            colors = colors.expand(points.shape[0], -1, -1)
            colors = [colors[b][filters[b]]
                        for b in range(colors.shape[0])]

        dirs = point_clouds.dirs_padded()
        if dirs is not None:
            dirs = dirs.expand(points.shape[0], -1, -1)
            dirs = [dirs[b][filters[b]]
                      for b in range(colors.shape[0])]

        weights = point_clouds.dirs_padded()
        if weights is not None:
            weights = weights.expand(points.shape[0], -1, -1)
            weights = [weights[b][filters[b]]
                    for b in range(colors.shape[0])]

        return PointClouds3D(points_list, normals=normals, features=features, colors=colors, dirs=dirs, weights=weights)
