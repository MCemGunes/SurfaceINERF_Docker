...
class PointsRasterizationSettings_hw:
    """
    Attributes:
    ...
    def __init__(
        ...
        points_per_local: Optional[int] = 8,
        num_shaping_points: Optional[int] = 20,
    ):
        ...
        self.points_per_local = points_per_local
        self.num_shaping_points = num_shaping_points


class SurfaceSplatting_hw_v2(PointsRasterizer):
    """
    ...
    def _filter_backface_points(self, point_clouds, **kwargs):
        ...
        colors_padded = point_clouds.colors_padded()
        dirs_padded = point_clouds.dirs_padded()
        weights_padded = point_clouds.weights_padded()
        normals_list = features_list = colors_list = dirs_list = weights_list = None
        ...
        if colors_padded is not None:
            colors_list = [colors_padded[b][mask[b]]
                             for b in range(colors_padded.shape[0])]
        if dirs_padded is not None:
            dirs_list = [dirs_padded[b][mask[b]]
                             for b in range(dirs_padded.shape[0])]
        if weights_padded is not None:
            weights_list = [weights_padded[b][mask[b]]
                             for b in range(weights_padded.shape[0])]
        ...
        new_point_clouds = point_clouds.__class__(
            points=points_list, normals=normals_list, features=features_list, colors=colors_list,
            dirs=dirs_list, weights=weights_list)
    ...
    def _filter_points_with_invalid_depth(self, point_clouds, **kwargs):
        ...
        with torch.autograd.no_grad():
            ...
            znear = getattr(self.cameras, 'znear', kwargs.get('znear', 0.1))
    ...
    def filter_renderable(self, point_clouds, point_clouds_filter=None, **kwargs):
        ...
        pc_check = point_clouds.clone()
        point_clouds, valid_depth_mask = self._filter_points_with_invalid_depth(
            point_clouds, **kwargs)
        # pdb.set_trace()
        # pc_check = point_clouds.clone()
        if point_clouds.isempty().item():
            pdb.set_trace()
            raise RuntimeError("Point cloud is empty after invalid depth filter.")
        ...
        if raster_settings.backface_culling:
            point_clouds, frontface_mask = self._filter_backface_points(
                point_clouds, **kwargs)
            vid = valid_depth_mask.clone()
            valid_depth_mask[vid] = frontface_mask
    ...
    def _compute_variance_and_detMk(self, pointclouds, **kwargs):
        ...
        pixel_size_h = 2.0 / raster_settings.image_size_h
        pixel_size_w = 2.0 / raster_settings.image_size_w
        variance = Vk + raster_settings.antialiasing_sigma * \
            ops3d.eyes(2, totalP, device=Vk.device,
                       dtype=Vk.dtype) * (pixel_size_h*pixel_size_w)
        # try:
        #     detMk = torch.det(Mk)
        # except:
        #     pdb.set_trace()
    ...
    def _get_per_point_info(self, pointclouds, **kwargs):
        ...
        GVs, _ = self._compute_variance_and_detMk(pointclouds, **kwargs)
        ...
        radii, cutoff_thres = self._get_ellipse_axis_aligned_radius(
            cutoff_thres, ellipseParams,
            # below params is useless ! We don't need to care!
            # min_radii=pixel_size, max_radii=20 * pixel_size
        )
    ...
    def _empty_fragments(self, batch_size, **kwargs):
        ...
        """
        raster_settings = kwargs.get("raster_settings", self.raster_settings)
        H = raster_settings.image_size_h
        W = raster_settings.image_size_w
        P = raster_settings.points_per_pixel
        idx = torch.full((batch_size, H, W, P), -1,
                         dtype=torch.long, device=self.device)
        zbuf = torch.full((batch_size, H, W, P), -1.0,
                          dtype=torch.float, device=self.device)
        qvalue_map = torch.full(
            (batch_size, H, W, P), -1.0, dtype=torch.float, device=self.device)
        occ_map = torch.full((batch_size, H, W), 0,
                             dtype=torch.float, device=self.device)
    ...
    def forward(self, point_clouds, point_clouds_filter=None, **kwargs) -> PointFragments:
        ...
        mask_filtered_clone = mask_filtered.clone()
        ...
        idx, zbuf, qvalue_map, occ_map, dx, dy, rx, ry = rasterize_elliptical_points(
            pcls_screen,
            ...
            image_size_h=raster_settings.image_size_h,
            image_size_w=raster_settings.image_size_w,
            ...
            points_per_local=raster_settings.points_per_local,
            num_shaping_points=raster_settings.num_shaping_points
        )
        ...
        fragments = PointFragments(
            idx=idx, zbuf=zbuf, qvalue=qvalue_map, occupancy=occ_map, dx=dx, dy=dy, rx=rx, ry=ry)

        # returns (P,) boolean mask for visibility
        visibility_mask = get_per_point_visibility_mask(
            point_clouds_filtered, fragments)
        mid = mask_filtered.clone()
        mask_filtered[mid] = visibility_mask
        ...
        if kwargs.get('verbose', False):
            ...
            return fragments, point_clouds_filtered, original_per_point_info, per_point_info['cutoff_threshold']

        return fragments, point_clouds_filtered, mask_filtered_clone, per_point_info['cutoff_threshold']
...
def rasterize_elliptical_points(pcls_screen, ellipse_params,
    ...
    if bin_size is None:
        ...
        if image_size_h <= 64:
            bin_size = 8
        elif image_size_h <= 256:
            bin_size = 16
        elif image_size_h <= 512:
            bin_size = 32
        elif image_size_h <= 1024:
            bin_size = 64
    if bin_size != 0:
        # There is a limit on the number of points per bin in the cuda kernel.
        num_bins = 1 + (image_size_h - 1) // bin_size
    ...
    idx, zbuf, qvalue_map, occ_map, dx, dy, rx, ry = EllipticalRasterizer.apply(
        points_packed, ellipse_params, cutoff_threshold, radii,
        cloud_to_packed_first_idx, num_points_per_cloud,
        depth_merging_threshold, image_size_h, image_size_w, points_per_pixel, bin_size, max_points_per_bin,
        radii_backward_scaler, points_per_local, num_shaping_points)
    return idx, zbuf, qvalue_map, occ_map, dx, dy, rx, ry
...
class EllipticalRasterizer(autograd.Function):
    ...
    def forward(ctx, pts_screen, ellipse_param, cutoff_threshold, radii,
        ...
        idx, zbuf, qvalue_map, occ_map, dx, dy, rx, ry = _C.splat_points_hw_v2(*args)
    ...
    def backward(ctx, idx_grad, zbuf_grad, qvalue_grad, occ_grad):
        ...
        grad_image_size_h = None
        grad_image_size_w = None
        ...
        grads = (grad_cutoff_thres, grad_radii, grad_cloud_to_packed_first_idx,
                 ...
                 grad_depth_merging_thres, grad_image_size_h, grad_image_size_w, grad_points_per_pixel,
