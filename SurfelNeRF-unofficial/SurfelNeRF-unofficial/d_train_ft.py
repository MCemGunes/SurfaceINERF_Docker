...
def do_engine(config, **kwargs):
    ...
    if distributed:
        if config.MODEL.FUSION.BUILD_FUSION:
    ...
    do_train(config, model, distributed, iteration=checkpointer.iter_)


def do_train(config, model, distributed, **kwargs):
    data_loader = build_dataloader(config, distributed=distributed)

    train_loop = Train_loop_online(config, model, data_loader, **kwargs)

    train_loop.train(**kwargs)
