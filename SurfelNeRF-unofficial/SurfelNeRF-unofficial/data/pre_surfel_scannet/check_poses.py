...
def main():
    ...
    for scene_name in tqdm(scene_name_list):
        ...
        if scene_name not in invalid_dict:
            ...
            for invalid_index in invalid_ids:
                for index in range(begin_id, invalid_index-10):
                    valid_list.append("{}__{}".format(scene_name, index))
                begin_id = invalid_index + 11
            for index in range(begin_id, num_txt):
                valid_list.append("{}__{}".format(scene_name, index))
        else:
            raise RuntimeError()
    #
    with open('./valid_index_scannet.pkl', 'wb') as f:
