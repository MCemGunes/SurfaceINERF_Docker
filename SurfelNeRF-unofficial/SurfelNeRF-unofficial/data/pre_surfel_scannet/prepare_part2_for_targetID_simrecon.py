...
def main(args):
    ...
    for i in tqdm(range(args.begin_id, args.end_id)):
        ...
        # for txt
        train_txt = os.path.join(args.txt_root, scene_name, args.txt_file)
        train_lines = txt_to_id(train_txt)
        for data_iter in tqdm(enumerate(train_lines), ):
            index, img_name = data_iter
            imgid = str(int(img_name.split('.')[0]))
            ...
            # depth_path = os.path.join(args.data_path, scene_name, 'depth_simplerecon', item_data['depth_img_name'])
            depth_path = os.path.join(args.data_path, scene_name, 'depth_simplerecon', item_data['depth_img_name'])
            #
