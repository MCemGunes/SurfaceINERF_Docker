...
from .frozenBN import FrozenBatchNorm2d
...
class _InvertedResidual_fixedBN(nn.Module):
    ...
    def __init__(
        ...
        super(_InvertedResidual_fixedBN, self).__init__()
        assert stride in [1, 2]
        assert kernel_size in [3, 5]
        mid_ch = in_ch * expansion_factor
        self.apply_residual = (in_ch == out_ch and stride == 1)
...
def _stack_fixedBN(in_ch: int, out_ch: int, kernel_size: int, stride: int, exp_factor: int, repeats: int,
           bn_momentum: float) -> nn.Sequential:
    """ Creates a stack of inverted residuals. """
    assert repeats >= 1
    # First one has no skip, because feature map size changes.
    first = _InvertedResidual_fixedBN(in_ch, out_ch, kernel_size, stride, exp_factor,
                                      bn_momentum=bn_momentum)
    remaining = []
    for _ in range(1, repeats):
        remaining.append(
            _InvertedResidual_fixedBN(out_ch, out_ch, kernel_size, 1, exp_factor,
                                      bn_momentum=bn_momentum))
...
class MNASNet_fixedBN(torch.nn.Module):
    """ MNASNet, as described in https://arxiv.org/pdf/1807.11626.pdf. This
    implements the B1 variant of the model.
    >>> model = MNASNet(1.0, num_classes=1000)
    >>> x = torch.rand(1, 3, 224, 224)
    ...
    def __init__(
        ...
        super(MNASNet_fixedBN, self).__init__()
        assert alpha > 0.0
        self.alpha = alpha
        self.num_classes = num_classes
        depths = _get_depths(alpha)
        layers = [
            # First layer: regular conv.
            nn.Conv2d(3, depths[0], 3, padding=1, stride=2, bias=False),  # 0
            FrozenBatchNorm2d(depths[0], ),  # 1
            nn.ReLU(inplace=True),  # 2
            # Depthwise separable, no skip.
            nn.Conv2d(depths[0], depths[0], 3, padding=1, stride=1,
                      groups=depths[0], bias=False),  # 3
            FrozenBatchNorm2d(depths[0], ),  # 4
            nn.ReLU(inplace=True),  # 5
            nn.Conv2d(depths[0], depths[1], 1, padding=0, stride=1, bias=False),  # 6
            FrozenBatchNorm2d(depths[1], ),  # 7
            # /2
            # MNASNet blocks: stacks of inverted residuals.
            _stack_fixedBN(depths[1], depths[2], 3, 2, 3, 3, _BN_MOMENTUM),  # 8
            # /4
            _stack_fixedBN(depths[2], depths[3], 5, 2, 3, 3, _BN_MOMENTUM),  # 9
            # /8
            # The below is useless
            _stack_fixedBN(depths[3], depths[4], 5, 2, 6, 3, _BN_MOMENTUM),  # 10
            # /16
            _stack_fixedBN(depths[4], depths[5], 3, 1, 6, 2, _BN_MOMENTUM),
            _stack_fixedBN(depths[5], depths[6], 5, 2, 6, 4, _BN_MOMENTUM),
            _stack_fixedBN(depths[6], depths[7], 3, 1, 6, 1, _BN_MOMENTUM),
            # Final mapping to classifier input.
            nn.Conv2d(depths[7], 1280, 1, padding=0, stride=1, bias=False),
            FrozenBatchNorm2d(1280,),
            nn.ReLU(inplace=True),
        ]
        self.layers = nn.Sequential(*layers)
        self.classifier = nn.Sequential(nn.Dropout(p=dropout, inplace=True),
    ...
    def _load_from_state_dict(self, state_dict: Dict, prefix: str, local_metadata: Dict, strict: bool,
        ...
        super(MNASNet_fixedBN, self)._load_from_state_dict(
            state_dict, prefix, local_metadata, strict, missing_keys,
            unexpected_keys, error_msgs)
...
def mnasnet1_0(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MNASNet:
    ...
    if pretrained:
        ...
        weights = torch.load('./data_src/init/mnasnet1.0_top1_73.512-f206786ef8.pth')  # yes! hard code!
        model.load_state_dict(weights)
    return model

def mnasnet1_0_fixedBN(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MNASNet:
    r"""MNASNet with depth multiplier of 1.0 from
    `"MnasNet: Platform-Aware Neural Architecture Search for Mobile"
    <https://arxiv.org/pdf/1807.11626.pdf>`_.
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    model = MNASNet_fixedBN(1.0, **kwargs)
    if pretrained:
        weights = torch.load('./data_src/init/mnasnet1.0_top1_73.512-f206786ef8.pth')  # yes! hard code!
