...
import math 
from .layers.blocks import ResNet_Block
from .layers.normalization import BatchNorm_StandingStats
from .configs import get_resnet_arch
from models.backbone.frozenBN import FrozenBatchNorm2d
    

def get_conv_layer(norm_G="sync:spectral_batch", use_3D=False):
...
def get_batchnorm_layer(norm_G="sync:spectral_batch", cfg=None):
    if cfg is not None:
        if cfg.MODEL.DEPTH.FIX_BN:
            norm_layer = FrozenBatchNorm2d
            return norm_layer
    norm_G = norm_G.split(":")[1]
    if norm_G == "batch":
...
# VGG architecture, used for the perceptual loss using a pretrained_stage1 VGG network
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        conv_layer = get_conv_layer(use_3D=use_3D)
        ...
        norm_layer = get_batchnorm_layer(cfg=opt)
