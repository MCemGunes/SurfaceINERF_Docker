...
def nerf_near_far_linear_ray_generation(campos,
    ...
    if jitter > 0.0:
        ...
        tvals = lower + (upper - lower) * t_rand
        # print("tvals, {}, t_rand {}, mids {}, upper {}, lower {}".format(tvals.shape, t_rand.shape, mids.shape, upper.shape, lower.shape))
    normalized_ray_dir = torch.linalg.norm(raydir[..., None, :], axis=-1)
    segment_length = torch.cat([tvals[..., 1:] - tvals[..., :-1], torch.full((tvals.shape[0], tvals.shape[1], 1), 1e10, device=tvals.device)], axis=-1)
    segment_length = segment_length * normalized_ray_dir
...
def near_far_linear_ray_generation(campos,
                                   ...
                                   jitter=0.,
                                   **kargs):
    # This method samples the points in the ray, and generate ray position based on the near, far hyper-parameters
    #
    # inputs
    # campos: N x 3
    # raydir: N x Rays x 3, must be normalized
    # point_cloud: opt.z_depth_dim, is the depth interval!
    # near: N x 1 x 1
    # far:  N x 1 x 1
    # jitter: float in [0, 1), a fraction of step length
    # outputs
    ...
    # For obtaining the right dim
    if len(campos.size()) > 2:
        campos = campos.squeeze(0)
    if len(raydir.size()) > 3:
        raydir = raydir.squeeze(0)
    #
    tvals = torch.linspace(0, 1, point_count + 1,
                           device=campos.device).view(1, -1)
    tvals = near * (1 - tvals) + far * tvals  # N x 1 x Sammples
    # segment_length is the depth interval, segment_length[now] = next_depth - now_depth
    segment_length = (tvals[..., 1:] - tvals[..., :-1]) * \
                     (1 + jitter * (torch.rand(
                          (raydir.shape[0], raydir.shape[1], point_count),
                          device=campos.device) - 0.5))  # the * factor can be regraded as 1
