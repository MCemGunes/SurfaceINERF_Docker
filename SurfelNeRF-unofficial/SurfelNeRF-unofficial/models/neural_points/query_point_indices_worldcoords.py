...
class lighting_fast_querier():
    ...
    def __init__(self, device, opt):
        ...
        self.claim_occ, self.map_coor2occ, self.fill_occ2pnts, self.mask_raypos, self.get_shadingloc, \
        self.query_along_ray = self.build_cuda()
    ...
    def get_hyperparameters(self, point_xyz_w_tensor, ranges=None):
        ...
        ranges_gpu, scaled_vsize_gpu, scaled_vdim_gpu, vscale_gpu, kernel_size_gpu = np_to_gpuarray(
            ranges_np, scaled_vsize_np, scaled_vdim_np, vscale_np, np.asarray(self.kernel_size, dtype=np.int32))
        ...
        return np.asarray(radius_limit_np).astype(np.float32), ranges_np, vsize_np, scaled_vsize_np, \
               scaled_vdim_np, vscale_np, ranges_gpu, \
               scaled_vsize_gpu, scaled_vdim_gpu, vscale_gpu, kernel_size_gpu

    def query_points(self, pixel_idx_tensor, point_xyz_w_tensor, actual_numpoints_tensor, h, w, near_depth, far_depth,
        ...
        range_gpu, scaled_vsize_gpu, scaled_vdim_gpu, vscale_gpu, kernel_size_gpu = self.get_hyperparameters(
        ...
        raypos_tensor, _, _, _ = near_far_linear_ray_generation(cam_pos_tensor, ray_dirs_tensor,
                                                                ...
                                                                jitter=0.1 if self.opt.IS_TRAIN else 0.)

        sample_pidx_tensor, sample_loc_w_tensor, ray_mask_tensor = self.query_grid_point_index(h, w, pixel_idx_tensor,
                                                                                               raypos_tensor,
                                                                                               point_xyz_w_tensor,
                                                                                               ...
                                                                                               ray_dirs_tensor,
                                                                                               cam_pos_tensor,
                                                                                               kMaxThreadsPerBlock=self.opt.COMMON.GPU_MAX_THREADS_PER_BLOCK)
        ...
        return sample_pidx_tensor, sample_loc_cam_coor_tensor, \
               sample_loc_w_tensor, sample_ray_dirs_tensor, ray_mask_tensor, vsize_np, ranges_np
    ...
    def build_cuda(self):
        ...
        b = torch.cuda.IntTensor(0)
        mod = SourceModule(
            """
            #define KN  """ + str(self.opt.MODEL.NEURAL_POINTS.K)
            + """
            #include <cuda.h>
            ...
            extern "C" {
                __global__ void claim_occ(
                    ...
                    //printf("i_batch %d and batch: %d ;", i_batch, B);
                    //if (i_batch >= B) { return; }
                    int i_pt = index - N * i_batch;
                    // printf("ipt %u while actual number is %d;", i_pt, in_actual_numpoints[i_batch]);
                    if (i_pt < in_actual_numpoints[i_batch]) {
                        ...
                        // printf("p_pt %f %f %f %f %f %f %f %f %f; ", d_voxel_size[0], d_voxel_size[1], d_voxel_size[2], floor(p_pt[0] - d_coord_shift[0])/d_voxel_size[0], (p_pt[1] - d_coord_shift[1]), (p_pt[2] - d_coord_shift[2]), coor[0], coor[1], coor[2]);
                        ...
                        // printf("coor_indx_b %u; ", coor_indx_b);
                        int voxel_idx = coor_2_occ[coor_indx_b];
                        // printf("voxel_idx %u; ", voxel_idx);
        ...
        query_along_ray = mod.get_function(
            "query_neigh_along_ray_layered")  # if self.opt.NN > 0 else mod.get_function("query_rand_along_ray")
    ...
    def build_occ_vox(self, point_xyz_w_tensor, actual_numpoints_tensor, B, N, P, max_o, scaled_vdim_np,
                      kMaxThreadsPerBlock, gridSize, scaled_vsize_gpu, scaled_vdim_gpu, kernel_size_gpu,
                      d_coord_shift):
    ...
    def query_grid_point_index(self, h, w, pixel_idx_tensor, raypos_tensor, point_xyz_w_tensor, actual_numpoints_tensor,
                               kernel_size_gpu, SR, K, ranges_np, scaled_vsize_np, scaled_vdim_np,
                               vscale_np, max_o, P, radius_limit_np, ranges_gpu, scaled_vsize_gpu,
                               scaled_vdim_gpu, vscale_gpu, ray_dirs_tensor, cam_pos_tensor, kMaxThreadsPerBlock=1024):
        ...
        d_coord_shift = ranges_gpu[:3]  # ranges of bbox, :3 means the min box
        ...
        self.mask_raypos(
            ...
            scaled_vdim_gpu,  # d_grid_size
            scaled_vsize_gpu,  # d_voxel_size
            Holder(raypos_mask_tensor),  # raypos_mask
            block=(kMaxThreadsPerBlock, 1, 1), grid=(gridSize, 1)
        )
...
def np_to_gpuarray(*args):
    ...
    for x in args:
        if isinstance(x, np.ndarray):
            result.append(gpuarray.to_gpu(x))
        else:
            print("trans",x)
    return result
