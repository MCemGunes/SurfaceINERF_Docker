...
def mse2psnr_float(x):
    if x == 0:
        return 0
    return -10.* math.log(x)/np.log(10.)
...
class Visualizer:
    def __init__(self, opt, log_path=None):
        self.opt = opt
        if log_path is None:
            log_path = os.path.join(opt.LOG.CHECKPOINT_PATH, opt.LOG.TASK_NAME)
        self.log_dir = log_path
        self.image_dir = os.path.join(self.log_dir, 'images')
        self.point_dir = os.path.join(self.log_dir, 'points')
        self.vid_dir = os.path.join(self.log_dir, 'vids')
        txt_dir = os.path.join(self.log_dir, 'log.txt')

        os.makedirs(self.vid_dir, exist_ok=True)
        # self.txt_writer = open(txt_dir, 'a')
    ...
    def reset(self):
        ...
        self.acc_iterations_psnr = 0
        self.acc_losses = OrderedDict()

    def accumulate_losses(self, losses):
        #TODO: check the key name in the losses

        self.acc_iterations += 1
        for k, v in losses.items():
            # self.acc_iterations_psnr += 1
            ...
            if k == "loss_color":
                ...
                self.acc_iterations_psnr += 1
                self.acc_losses[psnrkey] += mse2psnr_float(v)


    def get_psnr(self, key):
        return self.acc_losses[key + "_psnr"] / self.acc_iterations

    def print_losses(self, total_steps, **kwargs):
        m = "*"*50 + '\n'
    ...
    def write_dict(self, target_dict):
        m = ''
        for key in target_dict.keys():
            m += "{}: {}  ".format(key, target_dict[key])
        self.print_details(m)

    def plot_current_losses_with_tb(self, step, losses):
        if not self.opt.LOG.TENSORBOARD:
            return
    ...
    def obtain_output(self, output_dict):
        pass